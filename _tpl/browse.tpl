...
<?php
$start = isset($_GET['__start']) ? $_GET['__start'] : 0;
$start = (int) $start;
$start *= 250;
$end = $start + 250;
$limit_string = "$start, $end";

$result = $db->query('SELECT p1.id AS pID, p1.slug AS url, p1.revision, p1.title, p1.published, p1.updated, (SELECT COUNT(*) FROM pages WHERE slug = url AND visible = "y") AS revisionCount, (SELECT COUNT(*) FROM tests WHERE pageID = pID) AS testCount FROM pages p1 LEFT JOIN pages p2 ON p1.slug = p2.slug AND p1.updated < p2.updated WHERE p2.updated IS NULL AND p1.visible = "y" ORDER BY p1.updated DESC LIMIT ' . $limit_string);
