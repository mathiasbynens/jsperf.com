...
$benchmark = $showAtom = $jsClass = true;

require('tpl-inc/head.tpl');
...
if (in_array($item->browserscopeID, array(NULL, ''))) {
	$item->browserscopeID = addBrowserscopeTest($item->title, $item->info, $location->href);
...
<p class="meta"><?php
	if ($item->revision > 1) {
		?>Revision <?php echo $item->revision; ?> of this test case<?php
	} else {
		?>Test case<?php
	}
	?> created<?php echo author($item->author, $item->authorURL);
	?><time datetime="<?php echo date('c', strtotime($item->published));
	?>" pubdate><?php echo relativeDate($item->published);
	?></time><?php
	if (relativeDate($item->published) !== relativeDate($item->updated)) {
		?> and last updated <time datetime="<?php echo date('c', strtotime($item->updated)); ?>"><?php echo relativeDate($item->updated); ?></time><?php
	}
	if ($item->visible === 'n' && (isset($_SESSION['own'][$item->id]) || isset($_SESSION['admin']))) {
		?> <strong><a href="<?php echo $location->href; ?>/publish">Not published yet!</a></strong> · <a href="<?php echo $location->href; ?>/edit">Edit</a><?php
	}
?></p>
<?php if ($item->info) { ?>
<section>
<h2>Info</h2>
<?php echo md($item->info); ?>
</section>
<?php } ?>
...
<section id="runner">
<h2>Test runner</h2>
<p id="firebug"><strong>Warning! For accurate results, please disable Firebug before running the tests. <a href="<?php echo $location->origin; ?>/faq#firebug-warning">(Why?)</a></strong></p>
...
<?php foreach ($tests as $i => $test) { ?>
		<tr>
			<th scope="row" id="title-<?php echo ++$i; ?>"><div><?php
			echo addCode(he($test->title));
			if (strstr($item->initHTML, 'function init()')) {
				?> <span id="extra-<?php echo $i; ?>"></span><?php
			}
			?></div></th>
			<td class="code"><pre><code><?php
			if ('y' == $test->defer) {
				?><strong class="co1">// async test</strong><br><?php
			}
			echo highlight($test->code);
			?></code></pre></td>
...
<p>You can <a href="<?php echo $location->href; ?>/edit" rel="nofollow">edit these tests or add even more tests to this page</a> by appending <code>/edit</code> to the URL.</p>
<?php } ?>
</section>
...
<section>
<h2>Revisions</h2>
<p>You can <a href="<?php echo $location->href; ?>/edit" rel="nofollow">edit these tests or add even more tests to this page</a> by appending <code>/edit</code> to the URL. Here’s a list of current revisions for this page:</p>
<ul>
<?php
foreach ($revisions as $r) {
	if ($r->visible === 'y' || isset($_SESSION['admin'])) {
		?><li<?php if ($item->revision === $r->revision) { ?> class="current"<?php }
		?>><a href="<?php echo $location->origin . '/' . $slug; if ($r->revision > 1) { ?>/<?php echo $r->revision; }
		?>">Revision <?php echo $r->revision; ?></a>: published <?php if ($r->author) { echo 'by ' . he($r->author) . ' '; }
		?><time datetime="<?php echo date('c', strtotime($r->published));
		?>" pubdate><?php echo relativeDate($r->published);
		?></time><?php if (relativeDate($r->published) !== relativeDate($r->updated)) {
		?> and last updated <time datetime="<?php echo date('c', strtotime($r->updated));
		?>"><?php echo relativeDate($r->updated);
		?></time><?php } ?></li><?php
	}
} ?>
</ul>
