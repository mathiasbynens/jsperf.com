<?php flush(); ?>
<section id="comments"><h1><?php echo (count($comments) !== 1 ? count($comments) . ' comments' : '1 comment'); ?></h1><div id="comments-wrapper">
<?php $i = 0; foreach ($comments as $comment) { $owner = false; ?><article id="comment-<?php echo ++$i; ?>"<?php if ($comment->author === $item->author && $comment->authorEmail === $item->authorEmail && $comment->authorURL === $item->authorURL) { $owner = true; ?> class="owner"<?php } ?>><p class="meta"><img src="//gravatar.com/avatar/<?php echo md5($comment->authorEmail); ?>?s=26" width="26" height="26"> <?php echo author($comment->author, $comment->authorURL, true); if ($owner) { ?> (revision owner)<?php } ?> commented <time datetime="<?php echo date('c', strtotime($comment->published)); ?>" pubdate><?php echo relativeDate($comment->published); ?></time>: <a href="#comment-<?php echo $i; ?>" title="Permanent link to this comment">∞</a><?php if (isset($_SESSION['admin'])) { ?><a href="/edit-comment/<?php echo $comment->id; ?>">edit</a><?php } ?></p><div><?php echo md($comment->content); ?></div></article>
<?php } ?>
<?php flush(); ?>
